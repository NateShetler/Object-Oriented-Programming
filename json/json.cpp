...
std::string getAuthorTitle(std::string jstring)
{

  // Check to see if the string has anything in it
  if (jstring.length() == 0)
  {
    std::string empty = "No json data was given";
    return empty;
  }

  // String for the author and title
  std::string authorAndTitle = "";
  // This is a string to use to chop up the json
  std::string subStr = "";

  while(jstring.find(" ") != -1)
  {

    // Find the first element in the json
    subStr = subStr.substr(0, subStr.find(" "));
    jstring = jstring.substr(subStr.find(" ") + 1);

    // If the element is an author add it to the 
    // authorAndTitle string
    if (subStr.compare("\"author\":") == 0)
    {
      authorAndTitle += subStr;

      // Find the next element in the json
      subStr = subStr.substr(0, subStr.find(" "));
      jstring = jstring.substr(subStr.find(" ") + 1);
    }

    // If the element is a title add it to the 
    // authorAndTitle string
